...
fn main() -> std::io::Result<()> {
    ...
    let znc_log_dir = Path::new(&home_dir).join(".znc").join("moddata").join("log");

    let irc_log_dir = Path::new(&home_dir).join("irc-logs");

    fs::create_dir_all(&irc_log_dir)?;

    for network_dir in fs::read_dir(&znc_log_dir)? {
        let network_dir = network_dir?.path();

        if network_dir.is_dir() {
            let network_name = network_dir.file_name().unwrap().to_string_lossy();

            for channel_dir in fs::read_dir(&network_dir)? {
                ...
                if channel_dir.is_dir() {
                    ...
                    for log_file in fs::read_dir(&channel_dir)? {
                        ...
                        if log_file_path.is_file() && log_file_name.ends_with(".log") {
                            let date_parts: Vec<&str> = log_file_name.trim_end_matches(".log").split("-").collect();
